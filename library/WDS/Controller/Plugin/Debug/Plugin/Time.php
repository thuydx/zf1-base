 ...
 *
 */
class WDS_Controller_Plugin_Debug_Plugin_Time 
    ...
    implements WDS_Controller_Plugin_Debug_Plugin_Interface
{
    ...
    public function getLogger()
    {
        if (!$this->_logger) {
            $this->_logger = Zend_Controller_Front::getInstance()
                ->getPlugin('WDS_Controller_Plugin_Debug')->getPlugin('Log')->getLog();
    ...
    {
        return 'Time: ' . round($this->_timer['dispatchLoopShutdown']-$this->_timer['dispatchLoopStartup'],2). 'ms';
    }
    ...
    {
        $stats = $this->getDispatchStatistics();
        return $stats;
    }
    ...
    public function mark($name) {
        ...
        trigger_error("Debug Time plugin is deprecated, use the Log plugin");
    }
    ...
    public function getDispatchStatistics()
    {
        ...
        $timerNamespace = new Zend_Session_Namespace('Debug_Time',false);
        ...
        foreach ($timerNamespace->data as $module => $controller)
        {
            ...
            foreach ($controller as $con => $action)
            {
                ...
                foreach ($action as $key => $data)
                {
                    ...
                    $stats = ' Time/Request:  ' . $this->_calcAvg($data) . 'ms/'.count($data).' requests' .$this->getLinebreak();
