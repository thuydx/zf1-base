 ...
 *
 */
class WDS_Controller_Plugin_Debug_Plugin_Variables extends WDS_Controller_Plugin_Debug_Plugin implements WDS_Controller_Plugin_Debug_Plugin_Interface
{
    /**
    ...
    public function getPanel()
    {
        ...
        $server['Server OS'] = $_SERVER["OS"];       
        $server['Server Software'] = $_SERVER['SERVER_SOFTWARE'];       
        $server['User agent'] = $_SERVER["HTTP_USER_AGENT"];                    
        $server['Request Time'] = $_SERVER["REQUEST_TIME"];           
        $vars .= '<h4>Server</h4>'
        	   . '<div id="ZFDebug_cookie" style="margin-left:-22px">' . $this->_cleanData($server) . '</div>';
